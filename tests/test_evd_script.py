...
from core.handlers.evd_leaf import Arg
...
def script_bytes():
    return evd_leaf.compile_code(SCRIPT)


@pytest.fixture(scope="module")
def code(script_bytes):
    return evd_leaf.decompile_code(script_bytes, "Main")


@pytest.fixture
def lines(code):
    return evd_leaf.parse_code(code)
...
def test_kinds_and_depths(lines):
    ...
    assert kinds[1] == evd_leaf.KIND_EVENT
    assert kinds[2] == evd_leaf.KIND_DIRECTIVE
    assert kinds[5] == evd_leaf.KIND_LABEL
    assert kinds[6] == evd_leaf.KIND_COMMAND
...
def test_block_opener_knows_its_closing_brace(lines):
    ...
    assert lines[block.block_end - 1].kind == evd_leaf.KIND_CLOSE
    assert evd_leaf.block_range(lines, block.number) == (block.number, block.block_end)


def test_plain_line_owns_only_itself(lines):
    plain = line_with_head(lines, "set_flag")
    assert evd_leaf.block_range(lines, plain.number) == (plain.number, plain.number)
...
def test_category_follows_the_opcode(lines):
    assert line_with_head(lines, "if_value").category == evd_leaf.CATEGORY_JUMP
    assert line_with_head(lines, "set_value").category == evd_leaf.CATEGORY_EXPRESSION
    assert line_with_head(lines, "label").category == evd_leaf.CATEGORY_STRUCTURE
...
def test_double_slash_inside_a_string_is_not_a_comment():
    line = evd_leaf.parse_line('    print_text(text="a//b")  // real note', 1)
    assert line.arg("text") == '"a//b"'
    assert line.comment == "// real note"


def test_render_round_trips_unmodified_lines(code, lines):
    assert evd_leaf.render_code(lines) == code


def test_labels_and_references(lines):
    assert dict(evd_leaf.iter_labels(lines)) == {"loc_000C": 5}
    assert evd_leaf.label_references(lines).get("loc_000C") == 1  # the entry directive


###------------------------------------------- Compile errors -------------------------------------------###

def test_validate_accepts_the_fixture(code):
    assert evd_leaf.validate_code(code) is None


def test_validate_reports_the_authors_line(code):
    broken = code.replace("if_value(", "if_valve(")
    error = evd_leaf.validate_code(broken)
...
def test_an_unlocated_rejection_is_still_pinned_to_the_edited_line(lines):
    ...
    line = line_with_head(lines, "set_flag")
    result = evd_leaf.apply_line_edit(lines, line.number, (Arg("flag", "banana"),))
...
def test_compile_error_names_the_conflicting_derived_field():
    error = evd_leaf.EvdCompileError("character_sub_anim character_number does not match character", 7)
    assert error.conflicting_field == "character_number"
    assert evd_leaf.EvdCompileError("something else entirely").conflicting_field is None
...
def test_editing_an_input_rewrites_the_line(lines):
    ...
    result = evd_leaf.apply_line_edit(lines, line.number, (Arg("flag", "42"),))
    assert result.ok
...
def test_editing_an_input_drops_the_derived_field_it_invalidates(lines):
    ...
    result = evd_leaf.apply_line_edit(lines, line.number, edited)
    assert result.ok
    ...
    assert evd_leaf.validate_code(result.text) is None
...
def test_editing_out_of_range_is_rejected(lines):
    assert not evd_leaf.apply_line_edit(lines, 9999, ()).ok
...
def test_stale_name_annotations_are_dropped_on_edit(lines):
    ...
    result = evd_leaf.apply_line_edit(lines, line.number, (Arg("flag", "4400"),))
    assert result.ok
    assert "Movement Lock" not in result.changed_line


def test_hand_written_comments_survive_an_edit():
    kept = evd_leaf.prune_annotation("// my own note", {"flag"})
    assert kept == "// my own note"
    pruned = evd_leaf.prune_annotation("// flag=Movement Lock, item=Sword", {"flag"})
    assert pruned == "// item=Sword"
    assert evd_leaf.prune_annotation("// flag=Movement Lock", {"flag"}) == ""


###-------------------------------------------- Command index -------------------------------------------###

def test_index_loaded():
    assert evd_leaf.COMMANDS
    assert "set_flag" in evd_leaf.COMMANDS


def test_parameter_roles_are_available():
    info = evd_leaf.COMMANDS.get("control_character_sub_animation")
...
def test_aliases_resolve_to_the_printed_spelling():
    assert evd_leaf.COMMANDS.canonical("change_time") == "set_game_clock"
    assert evd_leaf.COMMANDS.get("change_time") is evd_leaf.COMMANDS.get("set_game_clock")
...
def test_every_documented_example_compiles_as_an_evdcode_call():
    ...
    failures = []
    for name in evd_leaf.COMMANDS.command_names:
        info = evd_leaf.COMMANDS.get(name)
        if info is None or not info.example:
            continue
        call = evd_leaf.example_to_call(info.example, indent=4)
        text = f"event Main {{\n    header(0x00000003)\n    entry(loc)\n    label(loc)\n{call}\n    end_script(yield=1)\n}}\n"
        if evd_leaf.validate_code(text) is not None:
...
def test_the_format_layer_carries_no_cli_or_container_code():
    """The handler is given raw EVD payloads and nothing else: decompression is
    another handler's job and there is no command line here, so neither a CLI,
    an SLZ container walk nor the analysis commands belong in this module."""
    for name in ("main", "build_parser", "SLZ_MAGIC", "container_summary",
                 "analyze_command_handlers", "cmd_decompile_code"):
        assert not hasattr(evd_leaf, name), f"{name} should have been stripped"


def test_the_naming_tables_are_filled_in():
    """The tables that name forms and parameters are populated after the literals
    that define them, by statements that bind no name -- `TABLE.update()` and
    `TABLE[key][key] = ...`. Losing those leaves a module that assembles identical
    bytes and prints different names, which is invisible, so they are pinned."""
    assert evd_leaf.PARAMETER_ALIASES["trigger"]["character_word"] == "character"
    assert evd_leaf.PARAMETER_ALIASES["window_message"]["message_id"] == "window"
    assert evd_leaf.FORM_NAME_ALIASES.get("change_time") == "set_radiata_time"
    assert len(evd_leaf.FORM_NAME_ALIASES) > 100
...
def test_the_battle_event_pair_names_a_file_and_a_script():
    ...
    info = evd_leaf.COMMANDS.get("configure_battle")
    assert info is not None
    ...
    assert evd_leaf.validate_code(BATTLE) is None


def test_the_pair_packs_into_one_word_high_half_first():
    data = evd_leaf.compile_code(BATTLE)
    code = evd_leaf.decompile_code(data, "Main")
    line = next(l for l in evd_leaf.parse_code(code) if l.head == "configure_battle")
    assert evd_leaf.parse_number(line.arg("battle_event_file")) == 500
    assert evd_leaf.parse_number(line.arg("battle_event_script")) == 3
...
def test_the_superseded_spellings_still_compile():
    ...
    assert evd_leaf.compile_code(old) == evd_leaf.compile_code(BATTLE)
...
def test_the_file_id_reads_as_an_event_name():
    ...
    assert evd_leaf.PARAMETER_SYMBOL_DOMAINS.get("battle_event_file") == "event"
...
def test_the_index_publishes_every_installed_opcode():
    ...
    published = {info.opcode for info in
                 (evd_leaf.COMMANDS.get(n) for n in evd_leaf.COMMANDS.command_names)
                 if info is not None and info.opcode is not None}
    assert set(evd_leaf.OPCODE_NOTES) - published == set()
...
def test_every_published_command_has_a_readable_summary():
    ...
    blank = [n for n in evd_leaf.COMMANDS.command_names
             if not (evd_leaf.COMMANDS.get(n).summary or "").strip()]
    assert not blank, f"commands with no summary: {blank}"
    raw_trace = [n for n in evd_leaf.COMMANDS.command_names
                 if re.match(r"^Command_[0-9a-f]{2}\b", evd_leaf.COMMANDS.get(n).summary or "")]
...
def test_every_published_parameter_has_a_description():
    ...
    blank = [f"{name}.{p.name}" for name in evd_leaf.COMMANDS.command_names
             for p in evd_leaf.COMMANDS.get(name).parameters if not (p.meaning or "").strip()]
...
def test_raw_examples_are_short_enough_to_read():
    ...
    oversized = [name for name in evd_leaf.COMMANDS.command_names
                 if evd_leaf.COMMANDS.get(name).raw and len(evd_leaf.COMMANDS.get(name).example) > 500]
...
def test_every_published_head_is_one_the_compiler_accepts():
    ...
    unwritable = [n for n in evd_leaf.COMMANDS.command_names
                  if not evd_leaf.is_writable_head(n)]
...
def test_handlers_without_a_form_still_carry_their_evidence():
    ...
    formless = [evd_leaf.COMMANDS.get(n) for n in evd_leaf.COMMANDS.command_names]
    ...
    for info in raw_with_evidence[:5]:
        assert info.opcode is None or info.opcode in evd_leaf.OPCODE_NOTES
...
def test_a_traced_meaning_is_told_apart_from_a_generated_one():
    ...
    vibration = evd_leaf.COMMANDS.get("play_vibration")
    ...
    generated = [(n, p.name) for n in evd_leaf.COMMANDS.command_names
                 for p in evd_leaf.COMMANDS.get(n).parameters if p.evidence == "template"]
...
def test_multi_operation_handlers_use_neutral_names():
    ...
    for old, new in (
        ...
        assert new in evd_leaf.COMMANDS, new
        assert evd_leaf.COMMANDS.canonical(old) == new, f"{old} should resolve to {new}"
        assert evd_leaf.COMMANDS.get(old) is evd_leaf.COMMANDS.get(new)


def test_the_neutral_summaries_say_what_else_the_handler_does():
    portrait = evd_leaf.COMMANDS.get("control_portrait")
    assert "erases" in portrait.summary
    camera = evd_leaf.COMMANDS.get("control_camera_animation")
...
def test_the_narrow_verbs_still_compile():
    ...
    assert evd_leaf.compile_code(f"{head}    {old}\n{tail}") == evd_leaf.compile_code(f"{head}    {new}\n{tail}")
...
def test_show_portrait_names_the_action_it_performs():
    ...
    info = evd_leaf.COMMANDS.get("control_portrait")
    assert info is not None
...
def test_play_vibration_middle_byte_is_one_bit_not_a_pattern():
    ...
    info = evd_leaf.COMMANDS.get("play_vibration")
...
def test_set_game_clock_day_is_one_based_with_two_sentinels():
    ...
    info = evd_leaf.COMMANDS.get("set_game_clock")
...
def test_packed_time_is_a_tuple_not_a_duration():
    ...
    info = evd_leaf.COMMANDS.get("set_time_schedule")
...
def test_the_listing_starts_at_zero_and_ends_at_the_file_size(script_bytes, code, lines):
    ...
    offsets = evd_leaf.assemble(code).offsets
...
def test_marker_table_is_a_directive_at_the_address_the_header_points_to():
    ...
    code = evd_leaf.decompile_code(data, "Main")
    lines = evd_leaf.parse_code(code)
    table = next(line for line in lines if line.head in ("markerTable", "marker_table"))
    assert table.kind == evd_leaf.KIND_DIRECTIVE
    assert evd_leaf.assemble(code).offsets[table.number] == evd_leaf.u32(data, 0x08) * 4
...
def test_every_label_sits_at_the_offset_its_name_says(code, lines):
    ...
    offsets = evd_leaf.assemble(code).offsets
    labels = [line for line in lines if line.kind == evd_leaf.KIND_LABEL]
    assert labels
    for line in labels:
        assert evd_leaf.label_for_offset(offsets[line.number]) == line.args[0].value


def test_commands_advance_by_their_own_size(code, lines):
    offsets = evd_leaf.assemble(code).offsets
    commands = [line for line in lines if line.kind == evd_leaf.KIND_COMMAND]
...
def test_a_label_takes_the_address_of_what_follows_it(code, lines):
    offsets = evd_leaf.assemble(code).offsets
    label = next(line for line in lines if line.kind == evd_leaf.KIND_LABEL)
    following = next(line for line in lines
                     if line.number > label.number and line.kind == evd_leaf.KIND_COMMAND)
...
def test_addresses_move_when_the_script_does():
    ...
    original = evd_leaf.decompile_code(evd_leaf.compile_code(
        "event Main {\n"
        "    header(0x00000003)\n"
        "    entry(start)\n"
        "    label(start)\n"
    ...
    target = [line for line in evd_leaf.parse_code(original) if line.kind == evd_leaf.KIND_LABEL][-1]
    name = target.args[0].value
    before = evd_leaf.assemble(original).offsets[target.number]
    assert evd_leaf.label_for_offset(before) == name, "the name matches the address before the edit"

    inserted = original.replace("    set_flag(flag=0)", "    set_flag(flag=0)\n    clear_flag(flag=1)", 1)
    moved = next(line for line in evd_leaf.parse_code(inserted)
                 if line.kind == evd_leaf.KIND_LABEL and line.args[0].value == name)
    after = evd_leaf.assemble(inserted).offsets[moved.number]
    assert after > before, "an inserted command must move what follows it"
    assert evd_leaf.label_for_offset(after) != name, "the name is stale, the address is live"


def test_assemble_reports_errors_without_offsets():
    built = evd_leaf.assemble("event Main {\n    not_a_command()\n}\n")
    assert not built.ok
...
def test_a_clean_script_reports_no_stranded_targets(lines):
    assert evd_leaf.undefined_label_problems(lines) == []
...
def test_deleting_a_label_strands_the_branch_that_used_it(code):
    ...
    assert evd_leaf.validate_code(branching) is None, "the compiler accepts it, which is the point"
    problems = evd_leaf.undefined_label_problems(evd_leaf.parse_code(branching))
...
def test_a_numeric_target_is_a_deliberate_raw_offset():
    lines = evd_leaf.parse_code('event Main {\n    jump(goto=0x0010)\n}\n')
    assert evd_leaf.undefined_label_problems(lines) == []
...
def test_numbers_compare_numerically_however_they_are_written():
    ...
    assert evd_leaf.value_key("1009") == evd_leaf.value_key("0x3F1")
    assert evd_leaf.value_key("1009") != evd_leaf.value_key("1010")
    assert evd_leaf.value_key('"text"') == '="text"'
    assert evd_leaf.value_key("event_value") is None, "a bare name is not a value"
    assert evd_leaf.value_key("") is None


def test_occurrences_are_values_not_names(lines):
    key = evd_leaf.value_key("1009")
    line = evd_leaf.parse_line("    set_value(event_value=1009, value=1009)  // note 1009", 1)
    spans = evd_leaf.value_occurrences(line, key)
...
def test_a_label_that_reads_like_a_value_is_not_one():
    line = evd_leaf.parse_line("    label(loc_1009)", 1)
    assert evd_leaf.value_occurrences(line, evd_leaf.value_key("1009")) == []
...
def test_a_parameter_name_is_never_an_occurrence():
    ...
    line = evd_leaf.parse_line("    set_flags(first_flag=4740, flag_count=2)", 1)
    spans = evd_leaf.value_occurrences(line, evd_leaf.value_key("2"))
    assert [line.text[a:b] for a, b in spans] == ["2"]


def test_occurrences_span_the_whole_script(code, lines):
    key = evd_leaf.value_key("1009")
    hits = [line.number for line in lines if evd_leaf.value_occurrences(line, key)]
...
def test_token_at_finds_the_word_under_a_column():
    ...
    assert evd_leaf.token_at(text, text.index("1009") + 1)[0] == "1009"
    assert evd_leaf.token_at(text, text.index("event_value") + 2)[0] == "event_value"
    assert evd_leaf.token_at(text, 0) is None


def test_hex_written_values_are_found_by_a_decimal_key():
    line = evd_leaf.parse_line("    set_value(event_value=0x3F1)", 1)
    assert evd_leaf.value_occurrences(line, evd_leaf.value_key("1009"))
...
def test_folding_hides_the_body_and_the_closing_brace(lines):
    block = line_with_head(lines, "if_value")
    hidden = evd_leaf.hidden_lines(lines, {block.number})
...
def test_a_fold_inside_a_fold_adds_nothing():
    ...
    assert evd_leaf.foldable(nested)[outer] > evd_leaf.foldable(nested)[inner]
    assert (evd_leaf.hidden_lines(nested, {outer, inner})
            == evd_leaf.hidden_lines(nested, {outer}))
...
def test_folding_never_touches_the_script(code, lines, script_bytes):
    ...
    for folded in ({}, {1}, set(evd_leaf.foldable(lines))):
        assert evd_leaf.render_code(lines) == code
        assert evd_leaf.compile_code(code) == script_bytes


def test_fold_summary_counts_the_hidden_lines(lines):
    block = line_with_head(lines, "if_value")
    summary = evd_leaf.fold_summary(lines, block.number)
...
def test_character_splits_into_an_id_and_a_variant():
    specs = evd_leaf.packed_spec(evd_leaf.COMMANDS.get("play_character_animation"), "character")
    assert specs is not None
    assert evd_leaf.split_packed(0x000326AC, specs) == {"character_number": 0x26AC, "character_variant": 0x03}
...
def test_the_split_uses_type_where_the_command_reads_a_type():
    ...
    typed = evd_leaf.packed_spec(evd_leaf.COMMANDS.get("control_character_sub_animation"), "character")
    plain = evd_leaf.packed_spec(evd_leaf.COMMANDS.get("play_character_animation"), "character")
...
def test_composing_only_touches_the_named_fields():
    specs = evd_leaf.packed_spec(None, "character")
    assert evd_leaf.compose_packed(0xAB0326AC, {"character_variant": 0x07}, specs) == 0xAB0726AC
    assert evd_leaf.compose_packed(0xAB0326AC, {"character_number": 1}, specs) == 0xAB030001
    ...
    assert evd_leaf.compose_packed(0xAB0326AC, {}, specs) == 0xAB0326AC


def test_a_plain_field_has_no_parts():
    assert evd_leaf.packed_spec(evd_leaf.COMMANDS.get("set_flag"), "flag") is None
...
def test_writing_the_parts_instead_of_the_word_silently_drops_the_character():
    ...
    assert evd_leaf.validate_code(parts) is None, "it compiles, which is what makes it dangerous"
    assert evd_leaf.compile_code(parts) != evd_leaf.compile_code(script)
    back = evd_leaf.decompile_code(evd_leaf.compile_code(parts), "Main")
...
def test_id_fields_offer_the_domain_the_decompiler_names_them_from():
    ...
    animation = evd_leaf.COMMANDS.get("play_character_animation")
    assert evd_leaf.symbol_domain(animation, "character_number") == "character"
    assert evd_leaf.symbol_domain(evd_leaf.COMMANDS.get("change_inventory"), "item_id") == "item"
    assert evd_leaf.symbol_domain(evd_leaf.COMMANDS.get("configure_battle"), "battle_bgm") == "bgm"
    assert evd_leaf.symbol_domain(evd_leaf.COMMANDS.get("configure_battle"), "battle_map") == "location"


def test_a_per_command_override_is_honoured():
    """`id` means a location on the background commands and nothing in general."""
    assert evd_leaf.symbol_domain(evd_leaf.COMMANDS.get("load_background"), "id") == "location"
    assert evd_leaf.symbol_domain(evd_leaf.COMMANDS.get("set_primitive_slot"), "id") is None
...
def test_the_packed_character_word_gets_no_picker():
    ...
    animation = evd_leaf.COMMANDS.get("play_character_animation")
    assert evd_leaf.symbol_domain(animation, "character") is None
    assert evd_leaf.symbol_domain(animation, "character_number") == "character"


def test_flags_are_not_offered_as_a_list():
    """Names cover 175 of 8,191 flags, so a list would hide more than it shows."""
    assert evd_leaf.symbol_domain(evd_leaf.COMMANDS.get("set_flag"), "flag") is None
    assert "flag" not in evd_leaf.PICKABLE_DOMAINS


def test_character_choices_include_the_abstraction_codes():
    choices = dict(evd_leaf.domain_choices("character"))
...
def test_a_name_can_be_written_where_an_id_is_expected():
    """The decompiler annotates `character=1` with `// character=Jack`, and the
    compiler accepts that name back, resolving it through the shared asset
    tables."""
    code = (
        "event Main {\n"
        "    header(0x00000003)\n"
        "    entry(start)\n"
        "    label(start)\n"
        "    load_character(mode=0, character=Jack, modeling=0x000101FF)\n"
        "    end_script(yield=1)\n"
        "}\n"
    )
    built = evd_leaf.assemble(code)
    assert not built.error, built.error
    assert built.data == evd_leaf.assemble(code.replace("character=Jack", "character=1")).data


def test_a_name_that_belongs_to_several_ids_is_not_guessed():
    """A duplicated name must stay numeric rather than silently pick the first
    id that happened to carry it."""
    reverse = evd_leaf.symbol_reverse_tables()
    names = evd_leaf.SYMBOLS.tables["item"]
    counts = {}
    for name in names.values():
        counts[name.strip().lower()] = counts.get(name.strip().lower(), 0) + 1
    ambiguous = [n for n, c in counts.items() if c > 1]
    assert ambiguous, "expected the item table to contain at least one repeated name"
    for name in ambiguous:
        assert name not in reverse["item"], name


def test_every_pickable_domain_has_entries():
    for domain in evd_leaf.PICKABLE_DOMAINS:
        assert evd_leaf.domain_choices(domain), domain


###----------------------------------------- Insertion templates ---------------------------------------###

def test_every_palette_entry_has_a_template():
    missing = [name for name, _family, _summary in evd_leaf.COMMANDS.palette_entries()
               if not evd_leaf.command_template(name)]
...
def test_templates_assemble_where_they_can():
    ...
    tail = '    end_script(yield=1)\n}\n'
    stubs = []
    for name, _family, _summary in evd_leaf.COMMANDS.palette_entries():
        ...
        template = evd_leaf.command_template(name, 4)
        if evd_leaf.validate_code(head + template + "\n" + tail) is not None:
            stubs.append(name)
...
def test_block_templates_win_over_the_jump_example():
    ...
    template = evd_leaf.command_template("if_value", 4)
    assert template.splitlines()[0].rstrip().endswith("{")
    assert "goto=" not in template


def test_unique_label_avoids_collisions(lines):
    assert evd_leaf.unique_label(lines) == "loc_new"
    with_label = evd_leaf.parse_code(evd_leaf.render_code(lines) + "label(loc_new)\n")
    assert evd_leaf.unique_label(with_label) == "loc_new1"
...
def test_handler_rejects_a_bad_save(script_bytes):
    node = VfsNode(name="test.evd")
    handler = EVDHandler(script_bytes)
    with pytest.raises(evd_leaf.EvdCompileError):
...
def test_stats_count_what_the_toolbar_shows(script_bytes, lines):
    stats = evd_leaf.script_stats(script_bytes, lines)
    assert stats.byte_size == len(script_bytes)
    assert stats.command_count == sum(1 for l in lines if l.kind == evd_leaf.KIND_COMMAND)
...
def test_event_name_is_sanitised_to_an_identifier():
    ...
    assert evd_leaf.event_name_for("516_01.evd") == "Event_516_01_evd"
    assert evd_leaf.event_name_for("Main") == "Main"
    assert evd_leaf.event_name_for("") == "Main"
