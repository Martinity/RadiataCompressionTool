"""Vendored from rs_elf/tools/evd_tool.py -- do not edit by hand.
...
Upstream: 23443 lines, 486 top-level statements.
Vendored: 359 kept, 116 dropped as unreachable.
"""
...
OPCODE_NOTES: dict[int, dict[str, str]] = {
    ...
    0x37: {
        ...
        "evidence": "Command_37 resolves a character (arg bit 0 = explicit selector) and branches on word0: low16 = effect id, byte2 = mode (1 tail, 2 mosaic, 0xFF flag toggle), byte3 = sub mode (1 create, 2 delete, 3 clear-points/params). Inline attach-object names use the fixed four-word slot; see the branch trace in docs/evd_script_notes.md.",
    },
    0x38: {
    ...
    0xD2: {
        ...
        "evidence": "Command_d2 builds or reads a per-character script param holder (r_chrScriptParamHolder.h): mode arg>>2 selects building a battle-team member list (sieves), picking a free stand position (GetStandPosition + Ground_CanBeArranged, shuffled or sorted by distance to target), listing child characters, or reading one element; the result count/element is always written to the event value named by the selector word's high half. See the branch trace in docs/evd_script_notes.md.",
    },
    0xD3: {
...
def load_symbol_tables(path: Path) -> dict[str, dict[int, str]]:
    """Load id-to-name tables produced by tools/build_symbols.py."""
    ...
    for domain, entries in raw.items():
        ...
        for key, name in entries.items():
            try:
                ...
                continue
            # Spreadsheet cells can contain newlines and quotes; annotations are
            # appended to a source line, so collapse them to something safe.
            clean = re.sub(r"\s+", " ", name.replace('"', "'").replace(";", ",")).strip()
            if len(clean) > 60:
...
DEFAULT_SYMBOLS_PATH = Path(__file__).resolve().parent.parent / "docs" / "evd_symbols.json"
...
def symbol_reverse_tables() -> dict[str, dict[str, int]]:
    ...
    if DEFAULT_SYMBOLS_PATH.is_file():
        tables = load_symbol_tables(DEFAULT_SYMBOLS_PATH)
